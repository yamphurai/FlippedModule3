...
class ViewController: UIViewController {
    ...
    //part A: properties to track the min & max of x & y for the left eye
    ...
    //Part B:
    var isBlinking: Bool = false //track blinking
    
    //Part C: track left (0) to right (1) in x-direction (initially pupil is assumed to be in center)
    var gazeDirection: Float = 0.5
    
    
    @IBOutlet weak var gazeDirectionSlider: UISlider!  // slider for the gaze
    @IBOutlet weak var gazeAngle: UILabel!   //gaze angle
    ...
    func landmarksCompletionHandler(request:VNRequest, error:Error?){
        ...
        DispatchQueue.main.async {
            ...
            //Part one: iterate thru each face observation to access landmarks
            for faceObservation in results {
                if let landmarks = faceObservation.landmarks,
                   let leftEye = landmarks.leftEye,
                   let leftPupil = landmarks.leftPupil {
                    
                    //access the normalized points for the left eye. Also creating a safe Swift-friendly array "points"
                    // NormalizedPoints: positions of key points (x & y) within face in a normalized coordinate system (i.e. 0-1). The coordinate system
                    // is scale independent i.e. not affected by actual size or resolution of face in image
                    let points = Array(UnsafeBufferPointer(start: leftEye.__normalizedPoints, count: leftEye.pointCount))
                    
                    //Calcualte the min & max x & y values
                    let minX = points.map { $0.x }.min() ?? 0
                    let maxX = points.map { $0.x }.max() ?? 0
                    let minY = points.map { $0.y }.min() ?? 0
                    let maxY = points.map { $0.y }.max() ?? 0
                    
                    // Update the tracked extrema values if necessary
                    self.leftEyeX.min = min(self.leftEyeX.min, Float(minX))
                    self.leftEyeX.max = max(self.leftEyeX.max, Float(maxX))
                    self.leftEyeY.min = min(self.leftEyeY.min, Float(minY))
                    self.leftEyeY.max = max(self.leftEyeY.max, Float(maxY))
                    
                    //Part Two
                    let currentDiffY = Float(maxY - minY) //current diff in y-direction
                    let greatestDiffY = self.leftEyeY.max - self.leftEyeY.min  // greatest difference observed so far in the y-direction
                    print("Current diff: \(currentDiffY), Greatest diff: \(greatestDiffY)")
                    
                    //Part Three: if eye blinks, diff between max & min y values of the eye approach to 0 or very small
                    let closedEyeThreshold: Float = 0.0250  //threshold to detect a closed eye
                    
                    
                    //if y diff is below threshold, eye blinked
                    if currentDiffY < closedEyeThreshold {
                        self.isBlinking = true  //blink is detected
                        print("Blink detected")
                    } else {
                        self.isBlinking = false  //reset blinking status when eyes are open
                    }
                    
                    //Part Four: Calculate teh gaze direction if the left eye is not blinking
                    if !self.isBlinking {
                        
                        //safely access raw array of x & y coordinates which are in normalized form (0-1)
                        let pupilPoints = Array(UnsafeBufferPointer(start: leftPupil.__normalizedPoints, count: leftPupil.pointCount))
                        
                        //map over pupilPoints to create list of x values so as to find min value helping us to understand how far left is the pupil
                        let minPupilX = pupilPoints.map { $0.x }.min() ?? 0
                        
                        // Normalize gaze direction based on leftEyeX.min and leftEyeX.max
                        // if min & max are equal we get division by 0 so need to check this condition
                        if self.leftEyeX.max > self.leftEyeX.min {
                            
                            //Position of gaze: shift min x of pupil to 0 and divide it by range of max to get normalized value between 0 (far left) to 1 (far right)
                            self.gazeDirection = Float((minPupilX - CGFloat(self.leftEyeX.min)) / (CGFloat(self.leftEyeX.max - self.leftEyeX.min)))
                            
                            self.gazeDirection = min(max(self.gazeDirection, 0), 1) // Clamp gazedirection between 0-1 to keep it within intended range
                            print("Gaze Direction (0 is left, 1 is right): \(self.gazeDirection)")
                            
                            self.gazeDirectionSlider.value = self.gazeDirection //slide value based on teh gaze direction
                            
                            self.gazeAngle.text = String(format: "Gaze angle: %.2f", self.gazeDirection)
                        }
                    }
                }
            }
